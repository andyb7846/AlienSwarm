...
BEGIN_ENT_SCRIPTDESC( CNPC_Combine, CBaseCombatCharacter, "Combine" )
	DEFINE_SCRIPTFUNC( Extinguish, "Extinguish a burning combine." )
	DEFINE_SCRIPTFUNC_NAMED( ScriptIgnite, "Ignite", "Ignites the combine into flames." )
	DEFINE_SCRIPTFUNC_NAMED( ScriptGiveWeapon, "GiveWeapon", "Gives the combine a weapon." )
	DEFINE_SCRIPTFUNC_NAMED( ScriptDropWeapon, "DropWeapon", "Makes the combine drop a weapon." )
	DEFINE_SCRIPTFUNC_NAMED( ScriptRemoveWeapon, "RemoveWeapon", "Removes a weapon from the combine." )
END_SCRIPTDESC()
...
void CNPC_Combine::InputGiveWeapon( inputdata_t &inputdata )
{
...
void CNPC_Combine::ScriptGiveWeapon( const char *pszName )
{
...
bool CNPC_Combine::ScriptDropWeapon()
{
	if ( GetActiveWeapon() )
	{
		Weapon_Drop( GetActiveWeapon() );
...
bool CNPC_Combine::ScriptRemoveWeapon()
{
	if ( GetActiveWeapon() )
	{
		Weapon_Drop( GetActiveWeapon() );
...
void CNPC_Combine::ScriptIgnite( float flFlameLifetime )
{
	ASW_Ignite( flFlameLifetime, NULL, NULL );
}

void CNPC_Combine::ASW_Ignite( float flFlameLifetime, CBaseEntity *pAttacker, CBaseEntity *pDamagingWeapon ) {
...
void CNPC_Combine::Spawn( void )
{
	...
	m_flFieldOfView			= -ULTRA_NARROW;// indicates the width of this NPC's forward view cone ( as a dotproduct result )
	...
	IGameEvent * event = gameeventmanager->CreateEvent( "combine_spawn" );
	if ( event )
	{
...
void CNPC_Combine::HandleAnimEvent( animevent_t *pEvent )
{
	...
	{
		if ( nEvent == COMBINE_AE_BEGIN_ALTFIRE )
		{
			EmitSound( "Weapon_CombineGuard.Special1" );
			handledEvent = true;
		}
		else if ( nEvent == COMBINE_AE_ALTFIRE )
		{
			if( IsElite() )
			{
	...
	if( handledEvent )
	{
		m_iLastAnimEventHandled = nEvent;
	}
}
