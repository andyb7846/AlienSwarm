...
BEGIN_ENT_SCRIPTDESC( CNPC_MetroPolice, CBaseCombatCharacter, "MetroPolice" )
	DEFINE_SCRIPTFUNC( Extinguish, "Extinguish a burning metropolice." )
	DEFINE_SCRIPTFUNC_NAMED( ScriptIgnite, "Ignite", "Ignites the metropolice into flames." )
	DEFINE_SCRIPTFUNC_NAMED( ScriptGiveWeapon, "GiveWeapon", "Gives the metropolice a weapon." )
	DEFINE_SCRIPTFUNC_NAMED( ScriptDropWeapon, "DropWeapon", "Makes the metropolice drop a weapon." )
	DEFINE_SCRIPTFUNC_NAMED( ScriptRemoveWeapon, "RemoveWeapon", "Removes a weapon from the metropolice." )
END_SCRIPTDESC()
...
void CNPC_MetroPolice::Spawn( void )
{
	...
	IGameEvent * event = gameeventmanager->CreateEvent( "metropolice_spawn" );
	if ( event )
	{
...
void CNPC_MetroPolice::SpeakSentence( int nSentenceType )
{
	...
	{
		/*if ( m_pPrimaryBehavior == &m_FuncTankBehavior )
		{
			SpeakFuncTankSentence( nSentenceType );
			return;
		}*/

		if ( m_pPrimaryBehavior == &m_StandoffBehavior )
		{
			SpeakStandoffSentence( nSentenceType );
			return;
		}

		if ( m_pPrimaryBehavior == &m_AssaultBehavior )
		{
...
void CNPC_MetroPolice::ScriptGiveWeapon( const char *pszName )
{
...
bool CNPC_MetroPolice::ScriptDropWeapon()
{
	if ( GetActiveWeapon() )
	{
		Weapon_Drop( GetActiveWeapon() );
...
bool CNPC_MetroPolice::ScriptRemoveWeapon()
{
	if ( GetActiveWeapon() )
	{
		Weapon_Drop( GetActiveWeapon() );
...
void CNPC_MetroPolice::ScriptIgnite( float flFlameLifetime )
{
	ASW_Ignite( flFlameLifetime, NULL, NULL );
}

void CNPC_MetroPolice::ASW_Ignite( float flFlameLifetime, CBaseEntity *pAttacker, CBaseEntity *pDamagingWeapon ) {
...
void CNPC_MetroPolice::HandleAnimEvent( animevent_t *pEvent )
{
	...
	// Shove!
	if ( nEvent == AE_METROPOLICE_SHOVE )
	{
		OnAnimEventShove();
		return;
	}

	if ( nEvent == AE_METROPOLICE_BATON_ON )
	{
		OnAnimEventBatonOn();
		return;
	}

	if ( nEvent == AE_METROPOLICE_BATON_OFF )
	{
		OnAnimEventBatonOff();
		return;
	}

	if ( nEvent == AE_METROPOLICE_START_DEPLOY )
	{
		OnAnimEventStartDeployManhack();
		return;
	}

	if ( nEvent == AE_METROPOLICE_DRAW_PISTOL )
	{
		m_fWeaponDrawn = true;
		if( GetActiveWeapon() )
		{
	...
	if ( nEvent == AE_METROPOLICE_DEPLOY_MANHACK )
	{
